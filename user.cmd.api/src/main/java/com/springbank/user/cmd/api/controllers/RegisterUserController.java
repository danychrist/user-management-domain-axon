...
@RestController //indicate that this is rest controller application
@RequestMapping(path = "/api/v1/registerUser")
public class RegisterUserController {
    //CommandGateway: Command dispatching mechanism that will dispatch the register user command
    ...
    public ResponseEntity<RegisterUserResponse> registerUser(@Valid @RequestBody RegisterUserCommand command){
        ...
        command.setId(id);
        try{
            commandGateway.sendAndWait(command); //will change back to sendAndWait later
